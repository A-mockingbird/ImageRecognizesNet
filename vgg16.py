...
class vgg16(Object):
    ...
    def _build_network(self, is_training):
        ...
        prediction_classfication = self._build_tail_net(conv, scope="vgg16_classification")
        if is_training:
            loss = self.crossentropy_loss_compute(prediction_classification, self.label)
            self.vgg_results['loss'] = loss
        self.vgg_results['predictons'] = prediction_classification
    ...
    def _build_tail_net(self, input_conv, scope):
        with tf.name_scope(scope):
            ...
            fc1 = self.fc_op(fc_in, 4096, "fc_1")
